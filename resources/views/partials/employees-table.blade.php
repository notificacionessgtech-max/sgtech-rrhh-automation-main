...
<x-app-layout>
    ...
    {{-- MODAL REDISEÑADO - Solo reemplaza desde aquí --}}
    <div id="userDetailModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center p-4">
        <div class="relative w-full max-w-6xl bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-2xl transform transition-all">

            {{-- Header del Modal - Diseño Premium --}}
            <div class="relative bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 rounded-t-2xl px-8 py-6 shadow-lg">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeModal()" class="group bg-white bg-opacity-20 hover:bg-opacity-30 text-white rounded-xl p-2 transition-all duration-200 backdrop-blur-sm">
                ...
                <div class="absolute -bottom-4 left-8 bg-white rounded-lg shadow-lg px-4 py-2 flex items-center space-x-2" id="documentCount">
            ...
            <div class="mt-8 px-8 py-6 overflow-y-auto" style="max-height: calc(90vh - 200px);" id="user-detail-content">
            ...
            <div class="bg-gray-50 border-t border-gray-200 px-8 py-5 rounded-b-2xl flex justify-end items-center space-x-3">
                <button onclick="closeModal()" class="group px-6 py-2.5 bg-white border-2 border-gray-300 hover:border-gray-400 text-gray-700 rounded-xl font-semibold transition-all duration-200 flex items-center space-x-2 shadow-sm hover:shadow">
...
<script>
    ...
    function renderCompleteEmployeeData(employee) {
        ...
        if (employee.documents && employee.documents.length > 0) {
            // Mostrar conteo
            documentCount.innerHTML = `<i class="fas fa-file mr-1"></i> ${employee.documents.length} documento(s)`;
            ...
            employee.documents.forEach((doc, index) => {
                ...
                documentsContent += `
                <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg border ${index > 0 ? 'mt-2' : ''}">
                    <div class="flex-1">
                        <p class="font-medium">${docType}</p>
                        <p class="text-sm text-gray-500 truncate">${fileName}</p>
                    </div>
                    <a href="/storage/${doc.file_path}"
                       download="${fileName}"
                       class="ml-4 px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium transition whitespace-nowrap">
            ...
            html += createSection(
                'Documentos Adjuntos',
                'fas fa-file-alt',
                `<div class="space-y-2 max-h-60 overflow-y-auto">${documentsContent}</div>`
            );
        } else {
            documentCount.innerHTML = '<i class="fas fa-file mr-1"></i> Sin documentos';
        }
...
{{-- JAVASCRIPT ACTUALIZADO - Reemplaza la función createSection --}}
<script>
    // ... (mantén todo el código anterior hasta createSection)
    ...
    // ... (mantén todo el código hasta la sección de documentos)

    // ACTUALIZA la sección de documentos en renderCompleteEmployeeData:
    // Reemplaza la parte de documentos (línea ~9. DOCUMENTOS) con esto:
